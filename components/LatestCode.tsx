export default function LatestCode({ repositories }: { repositories: [any] }) {
  return (
    <div id="latest-code" className="bg-[#F1F1F1] -mt-40 dark:bg-gray-900 pb-40">
      ...
      <div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto px-10 lg:-mt-10 gap-y-20">
          ...
          {repositories && repositories.map((latestRepo, idx) => <GithubRepoCard latestRepo={latestRepo} key="idx" />)}
        </div>
      </div>
    </div>
  )
...
function GithubRepoCard({ latestRepo }: { latestRepo: any }) {
  return (
